...
for month in months:
    ...
    for day in days[i]:
        ...
        device_total.rename(columns = {'device_count': month+'-'+day}, inplace = True)
        device_home.rename(columns = {'completely_home_device_count':month+'-'+day}, inplace = True)
        device_rate.rename(columns = {'distancing':month+'-'+day}, inplace = True)
    i += 1
    print("Done",month,'!')
