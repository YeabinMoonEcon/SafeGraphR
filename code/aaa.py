...
WorshipPlace.to_csv('/Users/yeabinmoon/Dropbox (UH-ECON)/Thesis/data/WeekVisitors.csv')
...
for week in week_list:  
    ...
    df.to_csv('/Users/yeabinmoon/Dropbox (UH-ECON)/Thesis/data/temp/monthly/'+week+'_cbg.csv')
    print("Done",week,'!')
    print("%f seconds" % (time.time() - start_time))
...
visits_large_county.to_csv('/Users/yeabinmoon/Dropbox (UH-ECON)/Thesis/data/visits_large_county.csv')
...
temp = visits_large_county.copy()
temp.loc[:,'state'] = temp.loc[:,'FIPS'].str[:2]

b = (temp.loc[:,'state'] == '48') | (temp.loc[:,'state'] == '06') | (temp.loc[:,'state'] == '17') | (temp.loc[:,'state'] == '12')
c = temp.loc[b,:]
a = temp.groupby('state')['FIPS'].count()
a = a.reset_index()
